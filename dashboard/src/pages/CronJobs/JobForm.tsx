    ...
    <div className="space-y-6 animate-in fade-in duration-500 pb-24 lg:pb-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-brand-900">{id ? 'Edit Jadwal' : 'Buat Jadwal Baru'}</h1>
            <p className="text-sm text-brand-500">Atur pesan yang akan dikirim otomatis oleh AI.</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        ...
        <div className="lg:col-span-2 space-y-6">
          <form id="job-form" onSubmit={handleSubmit(onSubmit)} className="bg-white border border-brand-200 rounded-2xl p-6 shadow-sm space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <div>
                <label className="block text-sm font-semibold text-brand-700 mb-1.5 ml-1">Nama Tugas</label>
                <input
              ...
              <div>
                <label className="block text-sm font-semibold text-brand-700 mb-1.5 ml-1">Frekuensi Pengiriman</label>
                <select
                  {...register('frequency')}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5 border-b border-brand-100 pb-6">
              <div>
                <label className="block text-sm font-semibold text-brand-700 mb-1.5 ml-1">Waktu Pelaksanaan</label>
                <input
              ...
              <div>
                <label className="block text-sm font-semibold text-brand-700 mb-1.5 ml-1">Target Nomor (WA)</label>
                <input
            ...
            <div>
              <label className="block text-sm font-semibold text-brand-700 mb-1.5 ml-1">Pesan Utama (Atau Instruksi AI)</label>
              <p className="text-xs text-brand-500 mb-3 ml-1">
            ...
            <div className="flex justify-end pt-4">
              <button
                type="submit"
                disabled={isSaving}
                className="flex items-center gap-2 bg-emerald-600 text-white px-6 py-2.5 rounded-xl font-medium hover:bg-emerald-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed shadow-sm w-full md:w-auto"
              >
        ...
        <div className="sticky top-24">
          <div className="bg-slate-100 border-[8px] border-slate-800 rounded-[2.5rem] h-[550px] w-full max-w-[320px] mx-auto overflow-hidden flex flex-col shadow-2xl relative">
            ...
            <div className="flex-1 bg-[#E5DDD5] p-4 flex flex-col justify-end overflow-y-auto">
              <div className="w-full flex justify-end">
                <div className="bg-[#DCF8C6] p-3 rounded-xl rounded-tr-none shadow-sm max-w-[85%] text-sm text-slate-800 relative">
                  ...
                  <div className="text-right text-[10px] text-emerald-700/60 mt-1">10:00</div>
            ...
            <div className="bg-slate-50 h-16 w-full border-t border-slate-200 px-4 flex items-center justify-center text-slate-400 text-xs">
              <div className="bg-white w-full rounded-full h-10 border border-slate-200 flex items-center px-4">
                Ketik pesan...
              </div>
            </div>
          </div>
          
          <div className="text-center mt-4 text-xs text-brand-400 flex items-center justify-center gap-1">
            <Smartphone size={14} />
